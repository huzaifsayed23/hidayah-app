...
export default function SurahReaderPage() {
  ...
  const [fontSize, setFontSize] = useState(26); // Compact sweet spot for high-density reading
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="p-2 hover:bg-hidayah-secondary rounded-full transition-colors text-hidayah-dark/40 dark:text-hidayah-gold dark:opacity-100"
            >
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="text-center mb-16">
          ...
          {chapter.id !== 1 && chapter.id !== 9 && (
            <div className="text-4xl font-arabic text-hidayah-gold mb-12 select-none">
          ...
          {verses.map((verse, index) => {
            ...
            return (
              <motion.div
                ...
                className="group relative bg-[#F2E8DA] p-4 md:p-6 rounded-[32px] border border-hidayah-border/40 hover:border-hidayah-gold/60 transition-all duration-500 shadow-sm"
              >
                {/* Compact Ayah Number */}
                <div className="flex items-center gap-3 mb-4">
                  <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-[#E5D5C0] border border-hidayah-gold/30">
                    <span className="text-[8px] font-bold text-[#2D241E] uppercase tracking-widest">
                      Ayah {verseNum}
                    </span>
                  </div>
                  <div className="h-px flex-1 bg-hidayah-gold/10" />
                </div>
 
                {/* Arabic Text - Compact leading */}
                <div 
                  className="w-full mb-6 font-arabic text-black transition-all duration-500 hover:text-hidayah-gold mushaf-layout"
                  dir="rtl"
                  style={{ fontSize: `${fontSize}px` }}
                >
                  <span className="inline-block align-middle">{verse.text_indopak}</span>
                </div>
 
                {/* Translation - Tightened margin */}
                <div className="text-black text-[13px] md:text-base leading-relaxed font-sans font-normal tracking-normal text-left max-w-3xl pl-3 border-l-[1.5px] border-hidayah-gold/40">
