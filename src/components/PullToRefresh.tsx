...
export default function PullToRefresh({ onRefresh, children, className = "" }: PullToRefreshProps) {
  ...
  const handlePan = (event: any, info: PanInfo) => {
    ...
    // Using a 5px buffer for mobile sub-pixel scrolling
  ...
  return (
    <div className={`relative ${className}`}>
      {/* Pull Indicator Area */}
      <motion.div 
        className="absolute top-0 left-0 right-0 flex justify-center items-center overflow-hidden z-50 bg-[var(--color-hidayah-primary)]"
      ...
      >
        <div className="flex flex-col items-center gap-1 opacity-80 py-4">
          ...
          {pullDistance > 30 && !isRefreshing && (
            <span className="text-[9px] font-bold uppercase tracking-widest text-[var(--color-hidayah-gold)]">
